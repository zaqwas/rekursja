...
import lesson._02B_ArithmeticSeries.ArithmeticSeriesLesson.State;
...
class TextFrame {
    ...
    public void gotoPart(final int part) {
        ...
        Platform.runLater(new Runnable() {
            @Override
            public void run() {
                if (shownPart == 1 && part1PseudocodeTabDisabledProperty.get()) {
    ...
    public void initialize(final byte current, final State state) {
        assert 1 <= current && current <= 2;
        assert state != null;
        
        selectedPart = current;
        shownPart = state.part;
        ...
        Platform.runLater(new Runnable() {
            ...
            public void run() {
                ...
                if (state == ArithmeticSeriesLesson.State.SummaryShown) {
                    part2SummaryTabDisabledProperty.set(false);
                }
