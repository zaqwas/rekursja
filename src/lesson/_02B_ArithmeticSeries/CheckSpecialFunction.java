...
class CheckSpecialFunction extends SpecialFunctionBehavior {
    ...
    private String error = "Funkcja „sumaCiagu” nie została zaimlementowana prawidłowo "
    ...
    public SyntaxNode commit(Instance instance) throws ProgramError {
        ...
        long value = startSpecialFunction.getValue();
        long properValue = (value * (value+1)) >> 1;
        BigInteger properValueBigInt = BigInteger.valueOf(properValue);
        ...
        if (userValueBigInteger.compareTo(properValueBigInt) != 0) {
            console.append("Funkcja „sumaCiagu” zwróciła nie poprawny wynik.\n"
                    + "Zwrócona wartość sumaCiagu("+ value +") = " + userValueBigInteger + "\n"
                    + "Poprawna wartość sumaCiagu("+ value +") = " + properValue + "\n");
            throw new ProgramError(error, call.getLeftIndex(), call.getRightIndex());
        }
        
        console.append("Funkcja „sumaCiagu” zwróciła poprawny wynik.\n"
                    + "sumaCiagu("+ value +") = " + properValue + "\n");
    ...
    public StringCreator getStatusCreatorAfterCall(Instance instance) {
        String str = "Funkcja „sumaCiagu” zwróciła poprawny wynik";
